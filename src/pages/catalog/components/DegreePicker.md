...
6. When the program changes, the component clears the stored path, disables downstream state, and calls `syncRequirements()` to fetch the program requirements file.
7. Requirements are hydrated from IndexedDB after sync; if requirements are missing, the table shows a missing-state message.
