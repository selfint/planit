# DegreePicker

## Overview

Provides catalog, faculty, program, and path selection and renders program requirements scoped to the selected path.

## Template Structure

- Selection block with four stacked selects (catalog, faculty, program, path) and status text.
- Requirements table that lists course-bearing requirement nodes with a breadcrumb-style label.

## Data Flow

1. `DegreePicker()` loads all catalogs from IndexedDB and reads the last active selection from meta (catalog, faculty, program, path).
2. Catalog picker options are built from the catalog IDs stored in IndexedDB. The selected catalog drives the available faculty list and enables the faculty picker.
3. Faculty picker options are built from the selected catalog’s top-level keys (excluding label keys), and the picker stays disabled if no faculty keys exist.
4. Program picker options are built from the selected faculty’s keys (excluding label keys), and the picker stays disabled until a valid faculty is selected.
5. When the program changes, the component clears the stored path, disables downstream state, and calls `syncRequirements()` to fetch the program requirements file.
6. Requirements are hydrated from IndexedDB after sync; if requirements are missing, the table shows a missing-state message.
7. Path options are detected from the top-level requirements nodes whose `en` label includes “path”.
8. The path picker is always visible. It is disabled and shows the empty placeholder when no path options exist.
9. If paths exist, the path picker is required and the table shows a prompt until a path is selected.
10. Selecting a path persists `requirementsActivePath` and filters the requirements tree to the selected path’s nested nodes plus elective nodes ("elective" in `en`).
11. The table renders only requirement nodes that contain courses, using a space-separated breadcrumb label built from ancestor nodes.
12. If a selected path contains no course-bearing nodes, the table shows an empty-path message.

## Dependencies

- `src/lib/catalogSync.ts`
- `src/lib/requirementsSync.ts`
- `src/lib/requirementsUtils.ts`
- `src/lib/indexeddb.ts`

## Notes

- The path picker is always visible; it disables with a message when no paths exist.
- The table renders only requirement nodes that contain courses, using a space-separated path label.
