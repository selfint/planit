...
2. App bootstrap (`src/main.ts`) registers `initCatalogSync(...)`; this component listens to `planit:catalog-sync` (`CATALOG_SYNC_EVENT`) so refreshes trigger a full picker reload from local IndexedDB.
