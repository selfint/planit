...
- A responsive virtualized results viewport that renders linked `CourseCard` rows (3/4/5 cards per row by breakpoint) using TanStack Virtual.
...
5. Matching `CourseRecord` items are grouped into responsive rows and rendered through `@tanstack/virtual-core`, so only visible rows (plus overscan) are mounted in the DOM.
...
- Validates the results viewport and virtualized results container render correctly.
- Validates filter controls are rendered and suggestion pills are not present.
- Validates default page size is `all` and linked course cards render from queried results inside the virtualized container.
