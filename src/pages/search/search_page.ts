...
function renderResults(results: HTMLDivElement, courses: CourseRecord[]): void {
    const nodes = courses.map((course) => {
        ...
        anchor.className =
            `focus-visible:ring-accent/60 block h-[10rem] rounded-2xl focus-visible:ring-2 sm:h-[9.5rem] [content-visibility:auto] [contain-intrinsic-size:10rem] sm:[contain-intrinsic-size:9.5rem] ${availabilityClass}`.trim();
