...
1. `PlanPage(stateManagement)` clones the template and mounts `ConsoleNav`.
2. On hydrate, the page fetches persisted/local data through injected state management:
    - `stateManagement.plan.getPlanState()`
    - `stateManagement.courses.getCoursesPage(...)`
...
5. Persisted state is updated with `stateManagement.plan.setPlanState(...)` after move/resize operations.
